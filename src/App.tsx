...
export default function App() {
  ...
  // --- PERSISTENT STATE ---
  const [xp, setXp] = useLocalStorage('wr-xp', { physics: 0, chemistry: 0, maths: 0 });
      ...
      interval = window.setInterval(() => {
        if (activeTimer.type === 'shift' && activeTimer.id) {
          ...
          setBossFightRemaining(prev => {
            if (prev <= 1) {
              setActiveTimer(null);
              setBossFightsCount(c => c + 1);
              setDailyData(d => ({ ...d, bossFightCompleted: true }));
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-300 font-sans flex flex-col">
      ...
      <main className="flex-1 flex flex-col max-h-screen overflow-y-auto">
        <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
          ...
          {/* TOP-LEVEL MODULE */}
          <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
            {/* Background Accent */}
                    ...
                    return (
                      <div 
                        ...
                        className={cn(
                          ...
                          isLocked ? "bg-zinc-950/50 border-zinc-900 opacity-60" : "bg-zinc-900 border-zinc-800",
                          isActive && "border-zinc-700 shadow-lg"
                        )}
                      >
                        {isActive && (
                          <div className={cn("absolute top-0 left-0 w-1 h-full", SUBJECT_COLORS[subject])} />
                        )}

                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-3">
                            <span className="text-xs font-mono text-zinc-500 font-bold">{shift.id}</span>
                            <h3 className={cn("font-black tracking-tight", isLocked ? "text-zinc-600" : "text-white")}>
                              {shift.title}
                            </h3>
                          </div>
                          {isLocked ? (
                    ...
                    return (
                      <div key={subj}>
                        ...
                        <div className="h-2 w-full bg-zinc-950 rounded-full overflow-hidden">
                          <div 
                            className={cn("h-full transition-all duration-1000", SUBJECT_COLORS[subj])} 
                            style={{ width: `${percent}%` }}
                          />
                        </div>
                      </div>
                ...
                <div className="space-y-3">
                  ...
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3 text-sm">
                      <div className={cn("w-4 h-4 rounded-full flex items-center justify-center border", req.done ? "bg-orange-500 border-orange-500" : "bg-zinc-950 border-zinc-700")}>
                        {req.done && <CheckCircle2 className="w-3 h-3 text-zinc-950" />}
                      </div>
                      <span className={cn("font-bold text-xs tracking-wider", req.done ? "text-zinc-300" : "text-zinc-600")}>{req.label}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {activeTimer?.type === 'boss' && (
        <div className="fixed inset-0 bg-zinc-950/95 backdrop-blur-xl z-50 flex flex-col items-center justify-center">
